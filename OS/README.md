...
* 이론적으로 세마포어는 P와 V 연산을 통해 공유 자원에 세마포어 변수값(설정값)만큼의 프로세스/쓰레드가 접근할 수 있고, 뮤텍스는 lock과 unlock 오퍼레이션을 통해 하나의 프로세스/쓰레드가 접근할 수 있다.
* 원론적으로 세마포어는 여러 개의 프로세스/쓰레드의 공유 자료 객체 접근을 제어하기 위한 카운터 역할을 수행하는 IPC이다. 반면 뮤텍스는 여러 프로세스/쓰레드의 공유 자료 객체 접근을 제어하기 위한 상호 배제 락 메커니즘이다.
* 원론적으로 세마포어는 IPC이기때문에 프로세스간 통신이 자연스럽다. 뮤텍스의 경우 프로세스간 락을 수행하려면 PTHREAD_PROCESS_SHARED와 같은 속성을 더해줘야 한다. (뮤텍스의 디폴트는 쓰레드간 락이다.)
* 실구현상 뮤텍스는 대부분 futex라는 메커니즘으로 구현되어 있기 때문에 유저 스페이스에서 동작 속도가 굉장히 빠르다. 세마포어는 System V 세마포어와 POSIX 세마포어로 나뉘는데, 전자는 순수하게 커널의 시스템콜로 구현되며 후자는 futex로 구현된다. System V 세마포어가 가장 유연한 기능을 가지고 있지만 futex를 사용하는 POSIX 메커니즘에 비해 속도가 느리다.
