...
* 이메일은 IP 주소가 아니라 호스트 네임을 이용해서 전송하기 때문에 DNS는 사이트 간의 메일 전송을 지원하기 위해 특별한 기능을 포함한다. 특수 메일 교환(MX) DNS RR은 도메인 수신 이메일을 처리하는데 사용하는 메일 서버의 네임을 포함하도록 설정되어 있다. 사이트로 메일을 전송하기 전에 장비는 메시지를 어디로 보낼지 알기 위해 해당 사이트의 MX 레코드를 얻기 위한 네임 변환을 수행한다.
